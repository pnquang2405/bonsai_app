...
class AppClients extends DioForNative {
  factory AppClients({BaseOptions? options}) {
    _instance ??= AppClients._(baseUrl: AppEndpoint.BASE_URL_DEV);
    if (options != null) {
      _instance!.options = options;
    }
    _instance!.options.baseUrl = AppEndpoint.BASE_URL_DEV;
    return _instance!;
  }
  ...
  Future<void> _requestInterceptor(
    ...
    options.connectTimeout = const Duration(seconds: AppEndpoint.connectionTimeout);
    options.receiveTimeout = const Duration(seconds: AppEndpoint.receiveTimeout);
